      ...
      <div className="relative z-10 max-w-4xl mx-auto animate-in fade-in duration-1000 slide-in-from-bottom-8">
        <div className="text-center mb-16">
          ...
          <p className="mt-8 text-white/60 max-w-2xl mx-auto text-lg">
            Please read these rules carefully. Ignorance of the rules is not an excuse for breaking them.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-4">
          {ruleCategories.map((category, index) => {
            ...
            return (
              <div 
                key={index} 
                className="bg-black/40 backdrop-blur-md border border-amber-500/20 rounded-2xl overflow-hidden shadow-lg transition-all duration-300 hover:border-amber-500/40"
              >
                <button
                  onClick={() => setOpenIndex(isOpen ? null : index)}
                  className="w-full flex items-center justify-between p-6 text-left focus:outline-none group"
                >
                  <span className="font-[var(--font-cinzel)] font-bold text-lg sm:text-xl tracking-wider text-amber-100 group-hover:text-amber-400 transition-colors">
                    {category.title}
                  </span>
                  <div className={`transform transition-transform duration-300 text-amber-500 ${isOpen ? 'rotate-180' : ''}`}>
                ...
                >
                  <div className="p-6 pt-0 border-t border-white/5 text-white/70 whitespace-pre-wrap leading-relaxed text-sm sm:text-base">
                    {category.content}
                  </div>
                </div>
              </div>
